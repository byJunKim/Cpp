...
private:
    ...
    int probesCount;
    int activeCounter;
    //this function returns the next node in the direction towards the node to delete
    TreeNode* findNodeToDelete(TreeNode* &parentptr, string name);
    //returns the maximum node on the left subtree of the node to delete
    TreeNode* getLeftMax(TreeNode* ptr);
    //recursively reach the smallest tree node and start deleting the whole tree
    void recursiveClear(TreeNode* currentNode);
    //searches the whole tree and returns the number of probes
    void countProbes(TreeNode* node, string name);
    //returns the number of active nodes
    int numOfActives(TreeNode* ptr) const;
